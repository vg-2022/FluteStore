...
type OrderPayload = Omit<Order, 'order_id' | 'order_date' | 'order_status' | 'status_history' | 'adminComments'>;
...
async function createAdminNotification(orderId: string, total: number) {
    ...
    if (adminUsers && adminUsers.length > 0) {
        const notifications = adminUsers.map((admin: { user_id: string }) => ({
            ...
            message: `A new order (#${orderId}) has been placed for a total of ${new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR' }).format(total)}.`,
        }));
...
export async function createOrderInDatabase(newOrderData: OrderPayload): Promise<Order | null> {
    const statusHistory: OrderStatusHistoryItem[] = [{ status: 'Placed', date: new Date().toISOString() }];
    ...
    return data ? { ...data, status_history: parseStatusHistory(data.status_history), cart_items: data.cart_items } : null;
}

function parseStatusHistory(history: any): OrderStatusHistoryItem[] | string {
