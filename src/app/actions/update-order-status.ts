...
async function createAdminNotificationForStatusChange(
  orderId: string,
  status: OrderStatus
) {
  ...
  if (rpcError) {
    console.error(
      "Error fetching admin user IDs for status change via RPC:",
      rpcError
    );
    return;
  }
  ...
  if (adminUsers && adminUsers.length > 0) {
    ...
    if (notificationError) {
      console.error(
        "Error creating admin notifications for status change:",
        notificationError
      );
    }
  }
