  ...
  const dashboardStats = useMemo(() => {
    ...
    const validOrders = orders.filter(
      (o) =>
        ...
        o.order_status !== "Cancellation Pending"
    );
    ...
    const totalProductsSold = validOrders.reduce(
      (acc, order) =>
        acc +
    ...
    switch (timeRange) {
      ...
      default: // 'all'
        startDate =
          orders.length > 0
            ? new Date(orders[orders.length - 1].order_date)
            : now;
    }

    const filteredOrders = orders.filter(
      (o) => new Date(o.order_date) >= startDate
    );
    const filteredValidOrders = validOrders.filter(
      (o) => new Date(o.order_date) >= startDate
    );
    const filteredUsers = users.filter(
      (u) => u.created_at && new Date(u.created_at) >= startDate
    );
    const filteredIssueOrders = filteredOrders.filter((o) =>
      ["Cancelled", "Refunded", "Cancellation Pending"].includes(o.order_status)
    );

    const getAggregator = (grouping: "day" | "month") => (date: Date) =>
      grouping === "day"
    ...
    const aggregateItemsSold = () => {
      ...
      const aggregated = filteredValidOrders.reduce((acc, order) => {
        ...
        const itemsInOrder = order.cart_items.reduce(
    ...
    <div className="space-y-6">
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {dashboardStats.totalProductsSold}
            </div>
            <div className="text-xs text-muted-foreground">In valid orders</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-2">
          <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              <Select
                value={activeChart}
                onValueChange={(val) => setActiveChart(val as ChartMetric)}
              >
              ...
              <CardDescription>
                Showing {chartConfig[activeChart].label} for the selected
                period.
              </CardDescription>
            </div>
        ...
        <Card>
          ...
          <CardContent>
            {dashboardStats.sortedTopProducts.map(
              ({ product, quantity }) =>
                product && (
                  <div
