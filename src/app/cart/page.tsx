          ...
          if (verificationResult.isVerified) {
            ...
            if (newOrder) {
              ...
              toast({
                ...
                description:
                  "Your order could not be saved after payment. Please contact support.",
              });
            }
          } else {
            toast({
                    ...
                    if (item.customizations) {
                      for (const [key, value] of Object.entries(
                        ...
                        const customizationConfig =
                          pdpSettings.customizations.find(
                            (c) => c.label === key
                          );
                        if (customizationConfig?.options) {
                          const selectedOption =
                            customizationConfig.options.find(
                              (opt) => opt.value === value
                            );
                          if (selectedOption?.price_change) {
                    ...
                    return (
                      ...
                      >
                        ...
                        <Button
                          variant="ghost"
                          size="icon"
                          className="text-muted-foreground hover:text-destructive mx-auto sm:mx-0"
                          onClick={() =>
                            removeFromCart(
                              item.product.productId,
                              item.customizations
                ...
                <div className="flex justify-between">
                  ...
                  {shippingFee === 0 && subtotal > 0 ? (
                    <span className="line-through">
                      {formatPrice(shippingSettings.defaultRate)}
                    </span>
                  ) : null}
                  <span
                    className={
                      shippingFee === 0 && subtotal > 0 ? "text-green-600" : ""
                    }
                  >
                    {shippingFee === 0 && subtotal > 0
                      ? "FREE"
