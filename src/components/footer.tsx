...
function FooterLinkColumn({ column }: { column: FooterColumn }) {
    return (
        <div>
            <h4 className="font-semibold text-foreground mb-4 font-headline">{column.title}</h4>
...
export function Footer() {
  const { storeDetails, socialLinks, footerSettings, isLoading } = useSettings();
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-8">
          <div className="lg:col-span-2 pr-8">
            <Link href="/" className="flex items-center gap-2 mb-4">
              {storeDetails.logo ? (
                <Image src={storeDetails.logo} alt={`${storeDetails.name} Logo`} width={32} height={32} className="h-8 w-8 object-contain"/>
              ) : (
                <Logo className="h-8 w-8 text-primary" />
              )}
              <span className="font-bold font-headline text-xl text-foreground">{storeDetails.name}</span>
            </Link>
        ...
        <div className="mt-8 pt-8 border-t flex flex-col md:flex-row justify-between items-center text-sm">
            ...
            <div className="flex items-center space-x-4 mt-4 md:mt-0">
                {socialLinks.map(link => (
                    <SocialIcon key={link.id} href={link.href}>
