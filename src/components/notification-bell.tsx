...
export function NotificationBell() {
  ...
  const markAsRead = async (notificationId: number) => {
    ...
    const alreadyRead = notifications.find(n => n.id === notificationId)?.is_read;
    if (!alreadyRead) {
      ...
      <DropdownMenuContent className="w-80 md:w-96" align="end">
        ...
        <ScrollArea className="h-[40vh]">
            {notifications.length > 0 ? (
                notifications.map(n => (
                    <DropdownMenuItem key={n.id} className="items-start gap-3" onSelect={(e) => e.preventDefault()}>
                        {!n.is_read && <Circle className="h-2 w-2 mt-1.5 fill-primary text-primary flex-shrink-0" />}
                        <div className={n.is_read ? 'pl-5 opacity-70' : ''}>
                            <Link href={`/account/orders/${n.order_id}`} className="hover:underline" onClick={() => markAsRead(n.id)}>
